module.exports = Object.freeze({
  API_ERROR: 'Oops, something went wrong',
  BASE_URL_API: 'http://localhost:8085',
  PLACEHOLDER_IMAGE:
    'https://www.signfix.com.au/wp-content/uploads/2017/09/placeholder-600x400.png',
  PURCHASE_ENDPOINT: '/purchases',
  LINEITEM_ENDPOINT: '/lineitems',
  REVIEW_ENDPOINT: '/reviews',
  RUNNING_SHORTS_ENDPOINT: '/products?category=Running&type=Short',
  PATIENTS_ENDPOINT: '/patients',
  ENCOUNTER_ENDPOINT: '/encounters',
  ENCOUNTERS_ENDPOINT: '/patientId/encounters',
  CREATE_PATIENTS_ENDPOINT: 'http://localhost:8085/patients',
  UPDATE_PATIENTS_ENDPOINT: 'http://localhost:8085/patients/patientId',
  GOOGLE_CLIENT_ID:
    '912899852587-7996nh9mlpvpa2446q0il4f9hj5o492h.apps.googleusercontent.com' // ENTER CLIENT ID HERE
});
